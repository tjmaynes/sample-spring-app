...
class CrudHandler<T>(
        private val service: IService<T>
) : IHandler<T> {
    override suspend fun all(request: ServerRequest): ServerResponse =
            service.getAll(PaginatedRequest(
                    pageNumber = getIntegerOrElse(request.pathVariable("pageNumber")) { 0 },
                    pageSize = getIntegerOrElse(request.pathVariable("pageSize")) { 10 }
            )).fold(
    ...
    override suspend fun getById(request: ServerRequest): ServerResponse =
            service.getById(request.pathVariable("id")).fold(
                    { createNon200Response(it).awaitFirst() },
                    { ServerResponse.ok().bodyValue(it).awaitFirst() }
            )
